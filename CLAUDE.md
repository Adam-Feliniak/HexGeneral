...
node tools/png-to-grid.js --palette --format=list|gpl [--write]  # same palette as a hex list (Aseprite set_palette) or a .gpl file; --write puts it in gitignored dist/palette/
node tools/palette-import.js p.hex     # optional — reads a downloaded palette (.hex/.gpl/.txt/.pal/.png/.json) into a hex list, to audition sprites in someone else's palette
...
To place pixels deliberately instead of composing a silhouette from ellipses, an **Aseprite MCP server is wired up** (project-local scope, installed outside the repo — see `Documents/09-Przewodnik-developera.md` for locations and the reproduce-elsewhere steps). It is a scratchpad only: the char maps in `gen-sprites.js` stay the source of truth, and the drawing comes back via `png-to-grid.js`. Two rules that are easy to get wrong — draw **without an outline** (`outline()` converts the shape's own outer ring to `o`, it does not add a ring outside, so a drawn outline gets counted twice), and stay on-palette (off-palette colours return as `?`).

Palette discipline does not have to be manual, but the obvious mechanism is the wrong one — all verified against `diivi/aseprite-mcp` v1.6.0: `set_palette` alone enforces nothing in RGB mode, and `indexed` mode enforces the palette while breaking the way back (`get_pixels_rect` then returns the raw palette index as the R channel). What works is **RGB + `set_palette` + `quantize_to_palette` before export** — it snaps every pixel to the nearest palette colour and leaves alpha alone, so the semi-transparent drop shadow survives. Feed it `--format=list`. The same three calls with a *different* list (via `palette-import.js`) repaint an existing asset in a downloaded palette, which is how you audition palettes without drawing anything. One more trap: `draw_pixels` paints on the active layer and `import_image_as_layer` does not switch it, so pixels silently land on `Layer 1` *underneath* the imported art — and `set_layer` does not help, because every MCP call is a separate batch-mode Aseprite process. Import onto `Layer 1` and stay single-layer. Details: `Documents/09-Przewodnik-developera.md`.
