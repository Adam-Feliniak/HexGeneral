...
- **`captureTile`** (`empire.js`): wejście na pole zmienia jego właściciela. Jeśli pole ma złoże i zmienił się właściciel — wytyczana jest nowa droga (`establishRoad`). Jeśli zdobyto cudzą **stolicę** — natychmiastowa **aneksja całego imperium** (`conquerEmpire`): wszystkie pola, miasta i złoża pokonanego gracza przechodzą pod zwycięzcę, wszystkie jego pozostałe armie znikają z planszy, złoża dostają świeżo wytyczone drogi do nowego właściciela. Zdobyta stolica staje się zwykłym miastem (`capitalOf = -1`).
