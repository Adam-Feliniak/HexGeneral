...
- **`#mark-view-field`** — przełącznik znaczników miast i złóż (zwykłe / szczegółowe, klawisz **D**). W trybie zwykłym znacznik pojawia się tylko na heksie zasłoniętym jednostką, w szczegółowym na wszystkich miastach i złożach — szczegóły w [Grafice](07-Grafika-i-sprite-y.md). **Zawsze widoczny**, w odróżnieniu od tempa AI: tempo ma sens tylko z botami, a czytelność mapy dotyczy każdej partii. Trwałość inna niż u `aiSpeed`: preferencja idzie do `localStorage` (`hexgeneral.view`), bo gracz, który raz ją włączył, nie powinien włączać jej po każdym uruchomieniu gry. W zapisie gry jej nie ma, więc `SAVE_FORMAT` bez zmian.
